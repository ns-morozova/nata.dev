...
const CasePreview: React.FC<CasePreviewProps> = ({
    ...
    return (
        <div className="group relative rounded-2xl bg-white shadow-lg transition-all duration-500 hover:shadow-2xl overflow-hidden">
            {/* Градиентные эффекты */}
            <div className="absolute -right-12 -top-12 z-10 h-24 w-24 rounded-full bg-gradient-to-br from-teal-100 to-teal-300 opacity-0 blur-xl transition-opacity duration-700 group-hover:opacity-80"></div>
            ...
            <div className="absolute top-4 left-4 md:left-6">
                <div className="inline-flex items-center rounded-full bg-gradient-to-r from-teal-600 to-teal-500 px-3 py-1 text-xs font-medium text-white shadow-sm transition-transform duration-500 group-hover:scale-105">
                    {category}
                </div>
            </div>
            ...
            <div className="absolute bottom-0 w-full h-max bg-gray-900/70">
                <div className="px-4 py-3 md:px-6 md:py-4">
                    <h3 className="mb-2 text-xl font-bold text-white opacity-90 transition-all duration-500 group-hover:opacity-100 group-hover:text-teal-50">
                        {title}
                    </h3>
                    ...
                    <a
                        href={link}
                        className="inline-flex items-center text-sm font-medium text-teal-400 transition-all duration-500 group-hover:text-teal-300"
                    >
                        <span className="relative">
                            Подробнее о проекте
                            <span className="absolute bottom-0 left-0 h-px w-0 bg-teal-300 transition-all duration-500 group-hover:w-full"></span>
                        </span>
                        <svg
