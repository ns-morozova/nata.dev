...
const CasePreview: React.FC<CasePreviewProps> = ({
    ...
    return (
        <article className="group relative rounded-2xl bg-white shadow-lg transition-all duration-500 hover:shadow-2xl overflow-hidden z-10">
            {/* Градиентные эффекты */}
            <div className="absolute -right-12 -top-12 z-10 h-24 w-24 rounded-full bg-gradient-to-br from-pink-500/80 to-pink-900 opacity-0 blur-xl transition-opacity duration-700 group-hover:opacity-80"></div>
            ...
            <div className="absolute top-4 left-4 md:left-6">
                <div className="inline-flex items-center rounded-full bg-gradient-to-r from-pink-900 to-pink-500/80 px-3 py-1 text-xs font-medium text-white shadow-sm transition-transform duration-500 group-hover:scale-105">
                    {category}
                </div>
            </div>
